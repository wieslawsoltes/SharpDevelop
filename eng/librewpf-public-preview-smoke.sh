...
run_start_page_smoke() {
  local attempt="$1"
  local config_dir="$work_root/start-page-config-$attempt"
  local home_dir="$work_root/start-page-home-$attempt"
  local log_file="$work_root/start-page-$attempt.log"
  ...
  env \
    ...
    LIBREWPF_SHARPDEVELOP_START_PAGE_SMOKE=1 \
  ...
  if [[ "$exit_code" -eq 0 ]] \
    && grep -Fq 'LibreWPF StartPage smoke result=Success' "$log_file" \
    && grep -Fq 'addin=True view=StartPageViewContent' "$log_file" \
    && grep -Fq 'interaction=True solutionOpened=True closedOnOpen=True cleanup=True' "$log_file" \
    && grep -Fq 'LibreWPF WorkbenchStartup application exit code=0' "$log_file"; then
    grep -E 'StartPage smoke result=|application exit code=' "$log_file"
    return 0
  fi

  echo "StartPage smoke attempt $attempt did not reach the complete success state." >&2
  tail -n 160 "$log_file" >&2
  return 1
}

if ! run_start_page_smoke 1; then
  echo "Retrying StartPage once with a fresh HOME and SharpDevelop configuration directory..." >&2
  run_start_page_smoke 2
fi

run_search_and_replace_smoke() {
  local attempt="$1"
  local config_dir="$work_root/search-and-replace-config-$attempt"
  local home_dir="$work_root/search-and-replace-home-$attempt"
  local log_file="$work_root/search-and-replace-$attempt.log"
  local timeout_seconds="${LIBREWPF_SHARPDEVELOP_SMOKE_TIMEOUT_SECONDS:-90}"
  local pid watchdog_pid exit_code
  ...
  env \
    ...
    LIBREWPF_SHARPDEVELOP_CONFIG_DIR="$config_dir" \
    LIBREWPF_SHARPDEVELOP_TRACE_OPEN=1 \
    LIBREWPF_SHARPDEVELOP_SEARCH_AND_REPLACE_SMOKE=1 \
    LIBREWPF_SHARPDEVELOP_EXIT_AFTER_MS=30000 \
    "$dotnet_cmd" "$app_dll" >"$log_file" 2>&1 &
  pid=$!
  ...
  if [[ "$exit_code" -eq 0 ]] \
    && grep -Fq 'LibreWPF SearchAndReplace smoke result=Success' "$log_file" \
    && grep -Fq 'searchReady=True replaceReady=True shortcuts=True closed=True' "$log_file" \
    && grep -Fq 'LibreWPF WorkbenchStartup application exit code=0' "$log_file"; then
    grep -E 'SearchAndReplace smoke result=|application exit code=' "$log_file"
    return 0
  fi

  echo "SearchAndReplace smoke attempt $attempt did not reach the complete success state." >&2
  tail -n 160 "$log_file" >&2
  return 1
}

if ! run_search_and_replace_smoke 1; then
  echo "Retrying SearchAndReplace once with a fresh HOME and SharpDevelop configuration directory..." >&2
  run_search_and_replace_smoke 2
fi

run_class_diagram_smoke() {
  local attempt="$1"
  local config_dir="$work_root/class-diagram-config-$attempt"
  local home_dir="$work_root/class-diagram-home-$attempt"
  local log_file="$work_root/class-diagram-$attempt.log"
  local timeout_seconds="${LIBREWPF_SHARPDEVELOP_SMOKE_TIMEOUT_SECONDS:-90}"
  local pid watchdog_pid exit_code
  ...
  env \
    ...
    LIBREWPF_SHARPDEVELOP_CONFIG_DIR="$config_dir" \
    LIBREWPF_SHARPDEVELOP_TRACE_OPEN=1 \
    LIBREWPF_SHARPDEVELOP_CLASS_DIAGRAM_SMOKE=1 \
    LIBREWPF_SHARPDEVELOP_EXIT_AFTER_MS=30000 \
  ...
  if [[ "$exit_code" -eq 0 ]] \
    && grep -Fq 'LibreWPF ClassDiagram smoke result=Success' "$log_file" \
    && grep -Fq 'hosted=True presented=True saveReload=True cleanup=True' "$log_file" \
    && grep -Fq 'LibreWPF WorkbenchStartup application exit code=0' "$log_file"; then
    grep -E 'ClassDiagram smoke result=|application exit code=' "$log_file"
    return 0
  fi

  echo "ClassDiagram smoke attempt $attempt did not reach the complete success state." >&2
  tail -n 160 "$log_file" >&2
  return 1
}

if ! run_class_diagram_smoke 1; then
  echo "Retrying ClassDiagram once with a fresh HOME and SharpDevelop configuration directory..." >&2
  run_class_diagram_smoke 2
fi
